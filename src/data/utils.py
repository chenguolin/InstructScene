...
def parse_threed_front_scenes(
    ...
    if os.getenv("PATH_TO_SCENES"):
        print(f"Load pickled 3D-FRONT scenes from {os.getenv('PATH_TO_SCENES')}")
        ...
        scenes = sum(scenes, [])
        pickle.dump(scenes, open("dataset/InstructScene/threed_front.pkl", "wb"))
...
def parse_threed_future_models(
    ...
    if os.getenv("PATH_TO_3D_FUTURE_OBJECTS"):
        print(f"Load pickled 3D-FUTURE objects from {os.getenv('PATH_TO_3D_FUTURE_OBJECTS')}")
        ...
        pickle.dump(furnitures, open("dataset/InstructScene/threed_future_model.pkl", "wb"))
